  ...
  return (
    <nav style={{
      ...
      <div style={{ maxWidth: 1200, margin: "0 auto", padding: "0 24px", display: "flex", alignItems: "center", justifyContent: "space-between", height: 64 }}>
        ...
        <button onClick={() => setMobileOpen(!mobileOpen)} className="nav-mobile-btn" style={{ display: "none", background: "none", border: "none", color: "#fff", fontSize: 24, cursor: "pointer" }}>
          {mobileOpen ? "✕" : "☰"}
        </button>
      </div>
      {mobileOpen && (
        <div style={{ background: "rgba(15,23,42,0.98)", padding: "16px 24px", display: "flex", flexDirection: "column", gap: 8 }}>
          ...
          <button onClick={() => navigate('/portal')}
              style={{ background: "transparent", border: "1px solid rgba(255,255,255,0.2)", color: "#fff", padding: "12px 16px", borderRadius: 8, cursor: "pointer", fontSize: 15, fontWeight: 600, textAlign: "left" }}>
              Portail Client
          </button>
          <button onClick={() => navigate('/admin')}
              style={{ background: "transparent", border: "1px solid rgba(255,255,255,0.2)", color: "#fff", padding: "12px 16px", borderRadius: 8, cursor: "pointer", fontSize: 15, fontWeight: 600, textAlign: "left" }}>
...
function Hero({ setPage }: { setPage: (p: string) => void }) {
  return (
    <section style={{
      ...
      background: "linear-gradient(170deg, #1A2332 0%, #1B2A4A 40%, #1A2332 100%)",
      ...
      <div style={{ maxWidth: 800, textAlign: "center", position: "relative", zIndex: 1 }}>
        ...
        <p style={{ fontSize: 18, color: "rgba(255,255,255,0.6)", lineHeight: 1.7, margin: "0 0 12px", maxWidth: 700, marginLeft: "auto", marginRight: "auto" }}>
        ...
        <CountdownBlock />
        <p style={{ color: "rgba(255,255,255,0.35)", fontSize: 12, marginTop: 12, letterSpacing: 1 }}>AVANT LE PREMIER SÉMINAIRE</p>
      </div>
      ...
      <div style={{ display: "flex", gap: 12, justifyContent: "center", flexWrap: "wrap", marginTop: 48, position: "relative", zIndex: 1 }}>
        {SEMINARS.map((s) => (
          <div key={s.id} style={{
            ...
            <div>
              <div style={{ color: "#fff", fontSize: 13, fontWeight: 700 }}>{s.code} · {s.title.split(" ").slice(-1)}</div>
              <div style={{ color: "rgba(255,255,255,0.45)", fontSize: 11 }}>{s.week}</div>
            </div>
          </div>
        ))}
      </div>
...
function FormatSection() {
  ...
  return (
    <section ref={ref} style={{ background: "#1A2332", padding: "80px 24px", borderTop: "1px solid rgba(255,255,255,0.05)" }}>
      <div style={{ maxWidth: 900, margin: "0 auto", opacity: vis ? 1 : 0, transform: vis ? "none" : "translateY(30px)", transition: "all 0.8s ease" }}>
        ...
        <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fit, minmax(340px, 1fr))", gap: 24 }}>
          {steps.map((s, i) => (
            <div key={i} style={{ background: "rgba(255,255,255,0.03)", borderRadius: 16, padding: 32, border: "1px solid rgba(255,255,255,0.08)" }}>
...
function SeminarCard({ sem, onSelect, delay = 0 }: any) {
  ...
  return (
    <div ref={ref} style={{
      ...
      border: "1px solid rgba(255,255,255,0.08)", opacity: vis ? 1 : 0, transform: vis ? "none" : "translateY(40px)",
      transition: `all 0.7s ease ${delay}ms`,
    }}>
      <div style={{ background: sem.gradient, padding: "28px 28px 20px", color: "#fff" }}>
      ...
      <div style={{ padding: "24px 28px" }}>
        <div style={{ fontSize: 13, color: "rgba(255,255,255,0.6)", marginBottom: 12 }}>
          <strong style={{ color: "#fff" }}>Public cible :</strong> {sem.target}
        </div>
        <div style={{ fontSize: 13, color: "rgba(255,255,255,0.6)", marginBottom: 16 }}>
          <strong style={{ color: "#fff" }}>Accès :</strong> Sélection exclusive · Format intimiste
        </div>
        ...
        {expanded && (
          <div style={{ marginBottom: 16, animation: "fadeIn 0.3s ease" }}>
            <div style={{ fontSize: 13, fontWeight: 700, color: "#fff", marginBottom: 8 }}>Modules de formation :</div>
            {sem.modules.map((m: string, i: number) => (
              <div key={i} style={{ display: "flex", gap: 8, alignItems: "center", padding: "6px 0", borderBottom: "1px solid rgba(255,255,255,0.05)" }}>
                ...
                <span style={{ fontSize: 14, color: "rgba(255,255,255,0.8)" }}>{m}</span>
              </div>
            ))}
            <div style={{ marginTop: 12, fontSize: 13, fontWeight: 700, color: "#fff" }}>Points clés :</div>
            {sem.highlights.map((h: string, i: number) => (
              <div key={i} style={{ fontSize: 13, color: "rgba(255,255,255,0.6)", padding: "4px 0 4px 16px", position: "relative" }}>
        ...
        <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginTop: 8, paddingTop: 16, borderTop: "1px solid rgba(255,255,255,0.05)" }}>
          <div>
            <div style={{ fontSize: 24, fontWeight: 800, color: "#fff" }}>{fmt(PRICE)} <span style={{ fontSize: 13, fontWeight: 400, color: "rgba(255,255,255,0.5)" }}>FCFA</span></div>
          ...
          <button onClick={() => onSelect(sem.id)} style={{
            background: sem.gradient, color: "#fff", border: "none", padding: "12px 24px",
...
function SeminarsPage({ setPage, setSelectedSem }: any) {
  return (
    <section style={{ background: "#1A2332", minHeight: "100vh", paddingTop: 100, paddingBottom: 80 }}>
...
function PricingPage({ setPage, setSelectedSem }: any) {
  ...
  return (
    <section style={{ background: "linear-gradient(170deg, #1A2332, #1B2A4A)", minHeight: "100vh", paddingTop: 100, paddingBottom: 80 }}>
        ...
        <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fit, minmax(300px, 1fr))", gap: 24, alignItems: "start" }}>
          {offers.map((o, i) => (
            <div key={i} style={{
              ...
              {o.badge && <div style={{ position: "absolute", top: -12, right: 20, background: "#27AE60", color: "#fff", padding: "4px 14px", borderRadius: 100, fontSize: 13, fontWeight: 800 }}>{o.badge}</div>}
              <div style={{ fontSize: 14, fontWeight: 700, color: o.primary ? "rgba(255,255,255,0.8)" : "rgba(255,255,255,0.5)", letterSpacing: 1, textTransform: "uppercase", marginBottom: 8 }}>{o.name}</div>
              <div style={{ fontSize: 36, fontWeight: 800, color: "#fff", marginBottom: 4 }}>{o.price}</div>
              <div style={{ fontSize: 13, color: o.primary ? "rgba(255,255,255,0.7)" : "rgba(255,255,255,0.4)", marginBottom: 24 }}>{o.unit}</div>
              {o.features.map((f, j) => (
                <div key={j} style={{ display: "flex", gap: 8, alignItems: "center", padding: "8px 0", color: o.primary ? "#fff" : "rgba(255,255,255,0.7)", fontSize: 14 }}>
...
function InscriptionPage({ selectedSem }: any) {
  ...
  if (submitted) return (
    <section style={{ minHeight: "100vh", display: "flex", alignItems: "center", justifyContent: "center", background: "#1A2332", paddingTop: 80 }}>
  ...
  return (
    <section style={{ minHeight: "100vh", background: "#1A2332", paddingTop: 100, paddingBottom: 80 }}>
...
function Footer({ setPage }: any) {
  const navigate = useNavigate();
  return (
    <footer style={{ background: "#0B1120", padding: "48px 24px 24px", borderTop: "1px solid rgba(255,255,255,0.06)" }}>
      <div style={{ maxWidth: 1100, margin: "0 auto" }}>
        <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fit, minmax(200px, 1fr))", gap: 32, marginBottom: 32 }}>
          ...
          <div>
            <div style={{ color: "rgba(255,255,255,0.5)", fontSize: 12, fontWeight: 700, letterSpacing: 1, marginBottom: 12, textTransform: "uppercase" }}>Navigation</div>
            {["home", "seminaires", "tarifs", "inscription"].map((p) => (
              <div key={p} style={{ marginBottom: 8 }}>
                <button onClick={() => { setPage(p); window.scrollTo(0, 0); }} style={{ background: "none", border: "none", color: "rgba(255,255,255,0.6)", cursor: "pointer", fontSize: 14, padding: 0 }}>
                  {p === "home" ? "Accueil" : p.charAt(0).toUpperCase() + p.slice(1)}
                </button>
              </div>
            ))}
            <div style={{ marginBottom: 8 }}>
                <button onClick={() => navigate('/portal')} style={{ background: "none", border: "none", color: "rgba(255,255,255,0.6)", cursor: "pointer", fontSize: 14, padding: 0 }}>
                  Portail Client
                </button>
            </div>
            <div style={{ marginBottom: 8 }}>
                <button onClick={() => navigate('/admin')} style={{ background: "none", border: "none", color: "rgba(255,255,255,0.6)", cursor: "pointer", fontSize: 14, padding: 0 }}>
                  Admin
                </button>
            </div>
          </div>
          <div>
            <div style={{ color: "rgba(255,255,255,0.5)", fontSize: 12, fontWeight: 700, letterSpacing: 1, marginBottom: 12, textTransform: "uppercase" }}>Contact</div>
        ...
        <div style={{ borderTop: "1px solid rgba(255,255,255,0.06)", paddingTop: 16, display: "flex", justifyContent: "space-between", alignItems: "center", flexWrap: "wrap", gap: 8 }}>
          <div style={{ color: "rgba(255,255,255,0.3)", fontSize: 12 }}>© 2026 RMK × CABEXIA · Tous droits réservés</div>
          <div style={{ color: "rgba(255,255,255,0.3)", fontSize: 12 }}>Formation délivrée par CABEXIA – Cabinet d'Expertise en Intelligence Artificielle</div>
        </div>
      </div>
    </footer>
  );
...
export default function LandingPage() {
  ...
  return (
    <div style={{ fontFamily: "'DM Sans', 'Segoe UI', sans-serif", margin: 0, minHeight: "100vh", background: "#1A2332" }}>
      <style>{`
      ...
      {page === "home" && (
        <>
          ...
          <FormatSection />
          <section style={{ background: "#1A2332", padding: "80px 24px", borderTop: "1px solid rgba(255,255,255,0.05)" }}>
            <div style={{ maxWidth: 1100, margin: "0 auto" }}>
              <div style={{ textAlign: "center", marginBottom: 48 }}>
