...
// ─── FINANCE ───
function FinancePage({ participants, seminars, prices, expenses, refreshExpenses, budgetConfig, setBudgetConfig }: any) {
    ...
    <div style={{ marginTop: 32 }}>
      ...
      <div style={{ display: "flex", justifyContent: "flex-end", marginBottom: 16 }}>
        <button onClick={() => setShowBudgetConfig(!showBudgetConfig)} style={{ background:"rgba(0,0,0,0.05)", border:"none", padding:"8px 16px", borderRadius:8, cursor:"pointer", fontSize:13, fontWeight:600 }}>
          ⚙️ Configuration du budget standard
        </button>
      </div>
            ...
            {Object.keys(tempConfig).map(k => (
              <div key={k}>
                ...
                <input 
                  ...
                  style={inputS} 
                  value={tempConfig[k]} 
                  onChange={e => setTempConfig({...tempConfig, [k]: Number(e.target.value)})} 
                />
              </div>
            ))}
          </div>
...
export default function AdminDashboard() {
  ...
  const [budgetConfig, setBudgetConfig] = useState<any>(DEFAULT_BUDGET_CONFIG);
