...
test.describe('Admin dashboard', () => {
  ...
  test('admin login shows error for wrong credentials', async ({ page }) => {
    ...
    // Should show an error message. Regex covers both the French UI strings
    // and the raw English "Invalid login credentials" passed through from
    // Supabase when the backend doesn't localize the error.
    const errorMsg = page.locator('text=/erreur|incorrect|invalid|invalide|échoué/i').first();
